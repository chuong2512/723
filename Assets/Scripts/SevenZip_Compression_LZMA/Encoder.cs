...
namespace SevenZip.Compression.LZMA
{
	public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
	{
		...
		private uint GetOptimum(uint position, out uint backRes)
		{
			...
			byte indexByte2 = this._matchFinder.GetIndexByte((int)(-this._repDistances[0] - 1u - 1u));
			...
			while (true)
			{
				...
				indexByte2 = this._matchFinder.GetIndexByte((int)(-this.reps[0] - 1u - 1u));
		...
		public void CodeOneBlock(out long inSize, out long outSize, out bool finished)
		{
			...
			if (this.nowPos64 == 0L)
			{
				...
				byte indexByte = this._matchFinder.GetIndexByte((int)(-(int)this._additionalOffset));
			...
			while (true)
			{
				...
				if (optimum == 1u && num5 == 4294967295u)
				{
					...
					byte indexByte2 = this._matchFinder.GetIndexByte((int)(-(int)this._additionalOffset));
					...
					{
						byte indexByte3 = this._matchFinder.GetIndexByte((int)(-this._repDistances[0] - 1u - this._additionalOffset));
		...
		public void SetCoderProperties(CoderPropID[] propIDs, object[] properties)
		{
			...
			while ((ulong)num < (ulong)((long)properties.Length))
			{
				...
				case CoderPropID.NumFastBytes:
				{
					...
					int num5 = (int)obj;
					if (num5 < 5 || (long)num5 > 273L)
					{
						throw new InvalidParamException();
					}
					this._numFastBytes = (uint)num5;
					goto IL_26D;
				}
				case CoderPropID.MatchFinder:
				{
					...
					int num6 = Encoder.FindMatchFinder(((string)obj).ToUpper());
					if (num6 < 0)
					{
						throw new InvalidParamException();
					}
					this._matchFinderType = (Encoder.EMatchFinderType)num6;
				...
				case CoderPropID.NumThreads:
				{
					...
					this._dictionarySize = (uint)num7;
					int num8 = 0;
					while ((long)num8 < 30L)
					{
						if ((long)num7 <= (long)(1uL << (num8 & 31)))
						{
							break;
						}
						num8++;
					}
					this._distTableSize = (uint)(num8 * 2);
					goto IL_26D;
				}
